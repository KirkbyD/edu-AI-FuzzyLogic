...
int main(int argc, char** argv) {
	...
	for (unsigned long x = 0; x < imageWidth; ++x) {
		for (unsigned long y = 0; y < imageHeight; y++) {
			...
			if (ch == '_') {
				/*cModelObject* cmo = new cModelObject(g_vec_pGameObjects, "Sphere", "WallCube");
				cmo->scale = 0.75f;
				...
				cmo->scene = 1;*/
				FuzzySystem.AddObstacle(glm::vec3(xPos, 1.f, yPos));
			}
			else if (ch == 'w') {
...

	{
		...
		Entity->friendlyName = "Walls";

		//Transform Component
		cTransformComponent* Transform = dynamic_cast<cTransformComponent*>(fact_game_obj.CreateGameObject("[TRANSFORM]"));
		Transform->setOrientation(glm::vec3(0.0f, 0.0f, 0.0f));
		Transform->setPosition(glm::vec3(0.f));
		Transform->setScale(1.f);
		Entity->AddTransformComponent(Transform);

		nPhysics::iPhysicsComponent* x0Wall = pPhysicsManager->FactoryCreateBox(glm::vec3(15.f, 0.f, -2.f), glm::vec3(0.f), glm::vec3(16.f, 10.f, 3.f), Transform->getQuaternion(), 0.f);
		if (x0Wall != nullptr) {
			pPhysicsManager->AddComponent(x0Wall);
			Entity->AddPhysicsComponent(x0Wall);
			pPhysicsManager->WorldAddComponent(x0Wall);
		}
		nPhysics::iPhysicsComponent* x32Wall = pPhysicsManager->FactoryCreateBox(glm::vec3(15.f, 0.f, 32.f), glm::vec3(0.f), glm::vec3(16.f, 10.f, 3.f), Transform->getQuaternion(), 0.f);
		if (x32Wall != nullptr) {
			pPhysicsManager->AddComponent(x32Wall);
			Entity->AddPhysicsComponent(x32Wall);
			pPhysicsManager->WorldAddComponent(x32Wall);
		}
		nPhysics::iPhysicsComponent* z0Wall = pPhysicsManager->FactoryCreateBox(glm::vec3(-2.f, 0.f, 15.f), glm::vec3(0.f), glm::vec3(3.f, 10.f, 16.f), Transform->getQuaternion(), 0.f);
		if (z0Wall != nullptr) {
			pPhysicsManager->AddComponent(z0Wall);
			Entity->AddPhysicsComponent(z0Wall);
			pPhysicsManager->WorldAddComponent(z0Wall);
		}
		nPhysics::iPhysicsComponent* z32Wall = pPhysicsManager->FactoryCreateBox(glm::vec3(32.f, 0.f, 15.f), glm::vec3(0.f), glm::vec3(3.f, 10.f, 16.f), Transform->getQuaternion(), 0.f);
		if (z32Wall != nullptr) {
			pPhysicsManager->AddComponent(z32Wall);
			Entity->AddPhysicsComponent(z32Wall);
			pPhysicsManager->WorldAddComponent(z32Wall);
		}
	...
	std::uniform_real_distribution<float> angle_dist(0.f, 359.9f);

	// spawn something at the spawn point
	for (size_t i = 0; i < 10; i++) 	{
		cGameObjectFactory fact_game_obj;
		cComplexObject* Entity = dynamic_cast<cComplexObject*>(fact_game_obj.CreateGameObject("[COMPLEX]"));
		Entity->friendlyName = "Vehicle";

		//Transform Component
		cTransformComponent* Transform = dynamic_cast<cTransformComponent*>(fact_game_obj.CreateGameObject("[TRANSFORM]"));
		Transform->setOrientation(glm::vec3(90.0f, 0.0f, 0.0f));
		Transform->setPosition(glm::vec3(0.f));
		Transform->setScale(1.f);
		Entity->AddTransformComponent(Transform);
		...
		GraphicsComponent->meshName = "WorkerModel";
		GraphicsComponent->friendlyName = "Vehicle";
		GraphicsComponent->v_textureNames = { "Brass" };
		GraphicsComponent->v_texureBlendRatio = { 1.0f };
		GraphicsComponent->scale = 0.02;
		glm::vec3 pos = spawnLocs[i]->pos;
		pos.y += 1.2f;
		GraphicsComponent->positionXYZ = pos;
		Entity->AddModel(GraphicsComponent);

		nPhysics::iPhysicsComponent*  PhysicsComponent = pPhysicsManager->FactoryCreateCharacterController( GraphicsComponent->positionXYZ,
																											0.6 /*Radius*/, 
																											1.f /*height*/,
																											0.001 /*stepheight*/, 
		...
		Entity->SetOrientation(glm::vec3(0.f, angle_dist(generator), 0.f));

		FuzzySystem.AddVehicle(Entity);
		mainCamera->SetFollowOffset(glm::vec3(5.f));
		mainCamera->SetFollowObject(Entity);
	}

	currentNode = spawnLocs[0];
	...
	for (int index = 0; index < ::g_vec_pGameObjects.size(); index++)
	{
		...
		{
			if (::g_vec_pGameObjects[index]->friendlyName == "Sensors") {
				std::vector<glm::vec3> Positions = FuzzySystem.GetSensorPositions();
	...
	while (!glfwWindowShouldClose(_window)) {
		...
		//todo: move to main systems update loop
		FuzzySystem.Update();
		...
		for (int index = 0; index < ::g_vec_pGameObjects.size(); index++) {
			if (::g_vec_pGameObjects[index]->objectType == "Instanced")	{
				if (::g_vec_pGameObjects[index]->friendlyName == "Sensors") {
					std::vector<glm::vec3> Positions = FuzzySystem.GetSensorPositions();
					for (size_t i = 0; i < Positions.size(); i++) {
						instancedTransformComponent_map[::g_vec_pGameObjects[index]->friendlyName]->moveInstance(i, Positions[i]);
					}
					instancedTransformComponent_map[::g_vec_pGameObjects[index]->friendlyName]->updateOnGPU();
				}
			}
